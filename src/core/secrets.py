...
def get_secret(secret_name: str, project_id: str = None) -> str:
    ...
    try:
        if not project_id:
            ...
            if not project_id:
                raise ValueError("GCP_PROJECT_ID must be set when using Google Secret Manager")
        
        logger.debug(f"Attempting to retrieve secret {secret_name} from GSM")
        client = secretmanager.SecretManagerServiceClient()
        name = Supabase.SECRET_PATH_TEMPLATE.format(project_id=project_id, secret_name=secret_name)
        response = client.access_secret_version(request={"name": name})
        logger.info(LogMessages.SECRET_RETRIEVED_GSM.format(secret_name=secret_name))
        return response.payload.data.decode("UTF-8")
    except Exception as e:
        logger.warning(LogMessages.SECRET_ACCESS_ERROR.format(secret_name=secret_name, error=e))
    ...
    for secret_name in Supabase.REQUIRED_SECRETS:
        if secret_name not in os.environ:
            ...
            if secret_value:
                ...
                logger.info(LogMessages.SECRET_LOADED_GSM.format(secret_name=secret_name))
            else:
                logger.warning(LogMessages.SECRET_EMPTY_WARNING.format(secret_name=secret_name))
